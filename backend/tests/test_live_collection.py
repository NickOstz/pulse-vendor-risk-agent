...
def test_live_serp_attempt_is_traced_before_labeled_fallback(monkeypatch, live_client):
    ...
    assert terminal["metrics"]["source_count"] == 1
    ...
    assert [call["timeout"] for call in captured] == [20.0, 20.0, 20.0, 8.0]
    assert any(
    ...
    assert not any(trace["source_mode"] == "fallback" for trace in traces)
...
def test_unlocker_timeout_is_traced_while_fallback_completes_review(monkeypatch, live_client):
    ...
    assert terminal["metrics"]["source_count"] == 0
    assert any(
...
def test_unlocker_provider_error_body_is_failed_not_extracted(monkeypatch, live_client):
    def fake_post(url, *, headers, json, timeout):
        if json["zone"] == "test-unlocker-zone":
            return httpx.Response(
                ...
                text=(
                    "Request Failed (bad_endpoint): Requested site is not available "
                    "for immediate access mode in accordance with robots.txt."
                ),
            )
        return httpx.Response(200, request=httpx.Request("POST", url), json={"organic": []})
    ...
    failed_unlocker = next(trace for trace in traces if trace["product"] == "web_unlocker")
    assert terminal["metrics"]["urls_scraped"] == 0
    assert terminal["metrics"]["source_count"] == 0
    assert terminal["metrics"]["evidence_count"] == 0
    assert failed_unlocker["status"] == "failed"
    assert "bad_endpoint" in failed_unlocker["error"]
    assert evidence == []
...
def test_verified_live_cloudflare_quote_replaces_cached_copy_and_creates_scored_alert(monkeypatch, live_client):
    ...
    assert terminal["metrics"]["source_count"] == 1
    assert terminal["metrics"]["evidence_count"] == 1
    ...
    assert "1 live verified public-source signal" in brief
    assert "## Evidence Table" in brief
    assert "| Signal | Severity | Mode | Support | Source | Recommended action |" in brief
    assert "| Trust / security | medium | live | verified | https://www.cloudflare.com/trust-hub/" in brief
    assert "<h2>Evidence Table</h2><table>" in html_brief
    assert '<span class="severity severity-medium">medium</span>' in html_brief
...
def test_opt_in_llm_extraction_validates_live_source_and_counts_call(monkeypatch, live_client):
    ...
    assert "Cloudflare is a critical edge security vendor" in brief
...
def test_seeded_snowflake_runs_live_review_with_deepseek_and_no_fallback(monkeypatch, live_client):
    source_url = "https://www.snowflake.com/en/why-snowflake/snowflake-security-hub/"
    quote = "Snowflake Security Hub describes security, compliance, privacy, and trust controls."
    ...
    def fake_model_call(self, prompt):
        return json.dumps(
            {
                "vendor_id": "vendor-snowflake",
                "signal_type": "trust_security",
                "claim": "Snowflake publishes a security hub with compliance, privacy, and trust controls.",
                "supporting_quote": quote,
                "source_url": source_url,
                "source_type": "vendor_owned",
                "published_or_captured_at": "2026-05-27T08:00:00Z",
                "severity_hint": "medium",
                "confidence": 0.9,
                "recommended_action": "Review Snowflake's current security package before renewal.",
            }
        )
    ...
    enabled = live_client.patch("/api/companies/vendor-snowflake/agent", json={"agent_enabled": True})
    assert enabled.status_code == 200
    tick = live_client.post("/api/agents/tick")
    assert tick.status_code == 200
    scan_id = tick.json()["started_scan_ids"][0]

    terminal = _poll_until_terminal(live_client, scan_id)
    traces = live_client.get(f"/api/brightdata/traces?scan_id={scan_id}").json()
    evidence = live_client.get(f"/api/companies/vendor-snowflake/evidence?scan_id={scan_id}").json()
    alerts = live_client.get(f"/api/alerts?company_id=vendor-snowflake&scan_id={scan_id}").json()
    brief = live_client.post(
        "/api/briefs/vendor-review",
        json={"company_id": "vendor-snowflake", "scan_id": scan_id, "format": "markdown"},
    )
    ...
    assert terminal["metrics"]["llm_calls_used"] == 1
    assert terminal["metrics"]["verified_count"] == 1
    assert captured_urls == [source_url]
    assert {trace["source_mode"] for trace in traces} == {"live"}
    assert evidence[0]["source_url"] == source_url
    assert evidence[0]["support_status"] == "verified"
    assert alerts[0]["title"] == "Verified live trust security signal requires review"
    assert brief.status_code == 200
    assert "Vendor Risk Assessment Brief: Snowflake" in brief.json()["content"]


def test_missing_completed_snowflake_brief_is_rendered_on_demand(monkeypatch, live_client):
    source_url = "https://www.snowflake.com/en/why-snowflake/snowflake-security-hub/"
    ...
    def fake_post(url, *, headers, json, timeout):
        if json["zone"] == "test-unlocker-zone":
            return httpx.Response(
                200,
                request=httpx.Request("POST", url),
                text="Snowflake Security Hub describes security, compliance, privacy, and trust controls.",
            )
        return httpx.Response(200, request=httpx.Request("POST", url), json={"organic": []})

    monkeypatch.setattr("app.services.brightdata_client.httpx.post", fake_post)
    live_client.patch("/api/companies/vendor-snowflake/agent", json={"agent_enabled": True})
    ...
    with Session(db.engine) as session:
        for brief in session.exec(select(Brief).where(Brief.scan_id == scan_id)).all():
            session.delete(brief)
        session.commit()

    recovered = live_client.post(
        "/api/briefs/vendor-review",
        json={"company_id": "vendor-snowflake", "scan_id": scan_id, "format": "markdown"},
    )

    assert recovered.status_code == 200
    assert "Vendor Risk Assessment Brief: Snowflake" in recovered.json()["content"]
...
def test_serp_discovery_prioritizes_risk_pages_over_signin_and_help(monkeypatch, live_client):
    source_text = {
        "https://orvvit.com/security": "Security controls include encryption and privacy review.",
        "https://orvvit.com/terms": "Subscription terms describe renewal and billing requirements.",
    }
    monkeypatch.setenv("LLM_EXTRACTION_ENABLED", "true")
    monkeypatch.setenv("DEEPSEEK_API_KEY", "test-llm-key")
        ...
        if json["zone"] == "test-serp-zone":
            ...
            if "pricing" in search_url:
                return httpx.Response(
                    ...
                    json={
                        "organic": [
                            {"link": "https://orvvit.com/help/faq", "title": "FAQ"},
                            {"link": "https://orvvit.com/terms", "title": "Terms and subscription billing"},
                        ]
                    },
                )
            if "trust" in search_url:
                return httpx.Response(
                    200,
                    request=httpx.Request("POST", url),
                    json={
                        "organic": [
                            {"link": "https://orvvit.com/signin", "title": "Sign in"},
                            {"link": "https://orvvit.com/security", "title": "Security and privacy"},
                        ]
                    },
                )
            return httpx.Response(200, request=httpx.Request("POST", url), json={"organic": []})
        captured_urls.append(json["url"])
        return httpx.Response(200, request=httpx.Request("POST", url), text=source_text[json["url"]])

    def fake_model_call(self, prompt):
        if "orvvit.com/security" in prompt:
            return json.dumps(
                {
                    "vendor_id": company_id,
                    "signal_type": "trust_security",
                    "claim": "Orvvit describes encryption and privacy review controls.",
                    "supporting_quote": source_text["https://orvvit.com/security"],
                    "source_url": "https://orvvit.com/security",
                    "source_type": "vendor_owned",
                    "published_or_captured_at": "2026-05-27T08:00:00Z",
                    "severity_hint": "medium",
                    "confidence": 0.9,
                    "recommended_action": "Review Orvvit security controls before renewal.",
                }
            )
        return json.dumps(
            {
                "vendor_id": company_id,
                "signal_type": "pricing_terms",
                "claim": "Orvvit terms describe subscription renewal and billing requirements.",
                "supporting_quote": source_text["https://orvvit.com/terms"],
                "source_url": "https://orvvit.com/terms",
                "source_type": "vendor_owned",
                "published_or_captured_at": "2026-05-27T08:00:00Z",
                "severity_hint": "medium",
                "confidence": 0.9,
                "recommended_action": "Review Orvvit commercial terms before renewal.",
            }
        )
    ...
    created = live_client.post(
        ...
        json={
            "name": "Orvvit",
            "domain": "orvvit.com",
            "relationship_type": "community platform",
            "owner": "Community",
    ...
    assert terminal["metrics"]["urls_scraped"] == 2
    assert terminal["metrics"]["verified_count"] == 2
    assert captured_urls == ["https://orvvit.com/security", "https://orvvit.com/terms"]
    assert {item["source_url"] for item in evidence} == {
        "https://orvvit.com/security",
        "https://orvvit.com/terms",
    }
...
def test_invalid_opt_in_llm_extraction_records_failure_and_preserves_fallback(monkeypatch, live_client):
    ...
    assert terminal["metrics"]["verified_count"] == 0
    assert {item["support_status"] for item in trust_items} == {"failed_source"}
    assert not any(alert["title"] == "Live compliance posture captured for renewal review" for alert in alerts)
    assert "no verified evidence" in brief
    assert "| Trust / security | fallback | verified | https://www.cloudflare.com/trust-hub/" not in brief
        ...
        return json.dumps(
            {
                "vendor_id": "vendor-cloudflare-demo",
                "signal_type": "trust_security",
                "claim": "Cloudflare has certification evidence on its trust center.",
                "supporting_quote": "SOC 2 Type II and ISO 27001 certifications are listed.",
                "source_url": "https://www.cloudflare.com/trust-hub/",
                "source_type": "vendor_owned",
                "published_or_captured_at": "2026-05-26T08:00:00Z",
                "severity_hint": "medium",
                "confidence": 0.9,
                "recommended_action": "Request current certification artifacts.",
            }
        )
...
def test_unlocker_retries_empty_markdown_with_raw_capture(monkeypatch, live_client):
    captured_payloads: list[dict] = []

    def fake_post(url, *, headers, json, timeout):
        captured_payloads.append(json)
        if json["zone"] == "test-unlocker-zone":
            if json.get("data_format") == "markdown":
                return httpx.Response(200, request=httpx.Request("POST", url), text="   ")
            return httpx.Response(200, request=httpx.Request("POST", url), text="<h1>Stripe security</h1>")
        return httpx.Response(200, request=httpx.Request("POST", url), json={"organic": []})
    ...
    terminal = _poll_until_terminal(live_client, scan_id)
    traces = live_client.get(f"/api/brightdata/traces?scan_id={scan_id}").json()
    unlocker_trace = next(trace for trace in traces if trace["product"] == "web_unlocker")

    assert terminal["metrics"]["urls_scraped"] == 1
    assert unlocker_trace["status"] == "success"
    assert unlocker_trace["retry_count"] == 2
    ...
    snapshot = Path(os.environ["BRIGHTDATA_LIVE_SNAPSHOT_DIR"]) / f"{scan_id}-configured-source.txt"
    assert snapshot.read_text(encoding="utf-8") == "<h1>Stripe security</h1>"


def test_block_page_capture_is_rejected_after_retry_ladder(monkeypatch, live_client):
    def fake_post(url, *, headers, json, timeout):
        if json["zone"] == "test-unlocker-zone":
            return httpx.Response(200, request=httpx.Request("POST", url), text="Just a moment... captcha")
        return httpx.Response(200, request=httpx.Request("POST", url), json={"organic": []})
    ...
    assert terminal["metrics"]["urls_scraped"] == 0
    assert unlocker_trace["status"] == "failed"
    assert unlocker_trace["retry_count"] == 2
    assert "block page signature" in unlocker_trace["error"]
    assert not list(Path(os.environ["BRIGHTDATA_LIVE_SNAPSHOT_DIR"]).glob("*.txt"))


def test_serp_discovery_prioritizes_official_terms_over_guides_and_resources(monkeypatch, live_client):
    source_text = "Stripe Services Agreement contains subscription, billing, and renewal terms."
    captured_urls: list[str] = []
    ...
    def fake_post(url, *, headers, json, timeout):
        if json["zone"] == "test-serp-zone":
            ...
            if "pricing" in search_url:
                return httpx.Response(
                    ...
                    json={
                        "organic": [
                            {
                                "link": "https://stripe.com/nz/resources/more-effective-billing",
                                "title": "Billing guide",
                                "description": "Resources for pricing operations",
                            },
                            {
                                "link": "https://stripe.com/legal/ssa",
                                "title": "Stripe Services Agreement",
                                "description": "Legal terms for billing, payment, and subscription renewal",
                            },
                        ]
                    },
                )
            return httpx.Response(200, request=httpx.Request("POST", url), json={"organic": []})
        captured_urls.append(json["url"])
        return httpx.Response(200, request=httpx.Request("POST", url), text=source_text)
    ...
    created = live_client.post(
        ...
        json={
            "name": "Stripe",
            "domain": "stripe.com",
            "relationship_type": "payments platform",
            "owner": "Finance",
            "criticality": "critical",
    ...
    assert terminal["metrics"]["urls_scraped"] == 1
    assert captured_urls == ["https://stripe.com/legal/ssa"]


def test_adverse_media_prioritizes_incident_writeup_over_generic_security_report(monkeypatch, live_client):
    source_text = (
        "Security Breach in Stripe Repo: The pwn request vulnerability allowed unauthorized "
        "commits and token exfiltration."
    )
    captured_urls: list[str] = []
            ...
            if "breach" in search_url:
                return httpx.Response(
                    ...
                    json={
                        "organic": [
                            {
                                "link": "https://www.reddit.com/r/stripe/comments/example_account_hacked/",
                                "title": "Stripe Account Hacked - Unauthorized Withdrawals",
                                "description": "User complaint says Stripe account hacked and unauthorized withdrawal incident.",
                            },
                            {
                                "link": "https://www.upguard.com/security-report/stripe",
                                "title": "Stripe Security Rating, Vendor Risk Report, and Data Protection",
                                "description": "Stripe security rating and vendor risk report.",
                            },
                            {
                                "link": "https://www.stepsecurity.io/blog/security-breach-in-stripe-repo-a-deep-dive-into-the-pwn-request-vulnerability",
                                "title": "A Deep Dive into the Pwn Request Vulnerability",
                                "description": "Security Breach in Stripe Repo involving unauthorized workflow token exfiltration.",
                            },
                        ]
                    },
                )
...
def test_adverse_media_ignores_promotional_security_vendor_reports(monkeypatch, live_client):
    captured_urls: list[str] = []
    ...
    def fake_post(url, *, headers, json, timeout):
        if json["zone"] == "test-serp-zone":
            ...
            if "breach" in search_url:
                return httpx.Response(
                    ...
                    json={
                        "organic": [
                            {
                                "link": "https://www.ionix.io/threat-center/vercel-security-incident/",
                                "title": "Vercel Security Incident",
                                "description": "Threat center report about breach, incident, and unauthorized access.",
                            },
                            {
                                "link": "https://guardz.com/blog/vercel-data-breach-driven-by-oauth-compromise/",
                                "title": "Vercel Data Breach Driven by OAuth Compromise",
                                "description": "Security vendor blog about incident and breach.",
                            },
                        ]
                    },
                )
            return httpx.Response(200, request=httpx.Request("POST", url), json={"organic": []})
        captured_urls.append(json["url"])
        return httpx.Response(200, request=httpx.Request("POST", url), text="Should not be fetched.")
